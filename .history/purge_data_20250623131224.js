...
async function purgePostgres(pgClient, tables) {
  console.warn('🔧 Deleting rows from PostgreSQL tables …');
  for (const t of tables) {
    try {
      await pgClient.query(`DELETE FROM ${t}`);
      console.error(`🗑️  Deleted rows from ${t} (Postgres)`);
    } catch (err) {
...
async function disableOracleFKs (conn) {
  console.info('🔧 Disabling Oracle FK constraints …');
  await conn.execute(`
    BEGIN
      FOR c IN (
...
async function enableOracleFKs (conn) {
  console.info('🔧 Re‑enabling Oracle FK constraints …');
  await conn.execute(`
    BEGIN
      FOR c IN (
        SELECT table_name, constraint_name
...
async function purgeOracle (conn, tables) {
  ...
  console.warn('🔧 Deleting rows from Oracle tables …');
  for (const t of tables) {
    await conn.execute(`DELETE FROM ${t}`);
    console.error(`🗑️  Deleted rows from ${t} (Oracle)`);
  }
  await conn.execute(`COMMIT`);

  await enableOracleFKs(conn);
  await conn.execute(`COMMIT`);
  console.success('✅ Oracle purge complete (constraints restored)');
}
...
async function purgeData () {
  ...
  try {
    ...
    console.success('✅ Connected to PostgreSQL');

    oracleConn = await oracledb.getConnection(oracleConfig);
    console.success('✅ Connected to Oracle');
