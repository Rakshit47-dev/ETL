...
/* ---------- new helper with success / error messages ---------- */
function safeReadLines(file) {
  try {
    ...
    process.exit(1);            // stop script on file‑read failure
  }
}
...
async function run() {
  ...
  try {
    ...
    const gradeRows   = safeReadLines('grade.txt');    // ← uses new helper
    ...
    const studentRows = safeReadLines('students.txt'); // ← uses new helper
